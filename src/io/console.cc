...
void ConsoleServer::Implementation::init() {
    ...
    if ( auto rc = _socket.bind(_socket_path) )
        ZEEK_AGENT_DEBUG("console-server", "opened socket {}", _socket_path.native());
    else
...
void ConsoleServer::Implementation::poll() {
    ...
    ZEEK_AGENT_DEBUG("console-server", "reading from socket {}", _socket_path.native());
    ...
    ZEEK_AGENT_DEBUG("console-server", "done reading from socket {}", _socket_path.native());
}
...
void ConsoleServer::Implementation::execute(socket::Remote remote, const std::string& cmd, bool terminate) {
    ...
    // server thread. For queries, we wll spawn a worker thread to serve them.
    if ( cmd == ".version" ) {
...
void ConsoleServer::start() {
    ...
    DWORD flags;
    GetConsoleServerMode(handle, &flags);
    flags |= ENABLE_PROCESSED_OUTPUT;
    flags |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;
    SetConsoleServerMode(handle, flags);
#endif
...
void ConsoleClient::Implementation::init() {
    filesystem::path client_socket = frmt("{}.{}", _socket_path.native(), getpid());
    ...
    ZEEK_AGENT_DEBUG("console-client", "opened socket {}", client_socket.native());
}
...
void ConsoleClient::Implementation::repl() {
    ...
    ZEEK_AGENT_DEBUG("console-client", "reading from socket {}", _socket_path.native());
    ...
    ZEEK_AGENT_DEBUG("console-client", "done reading from socket {}", _socket_path.native());
}
...
void ConsoleClient::start() {
    ...
    static const HANDLE handle = GetStdHandle(STD_OUTPUT_HANDLE);
    DWORD flags;
    GetConsoleClientMode(handle, &flags);
    flags |= ENABLE_PROCESSED_OUTPUT;
    flags |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;
    SetConsoleClientMode(handle, flags);
#endif
